...
export async function PUT(req: NextRequest) {
  ...
  const [existing] = await db.select({ id: categories.id }).from(categories)
    .where(and(eq(categories.id, id), eq(categories.storeId, storeId!))).limit(1)
  if (!existing) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const [cat] = await db.update(categories).set({ name: name.trim() }).where(eq(categories.id, id)).returning()
